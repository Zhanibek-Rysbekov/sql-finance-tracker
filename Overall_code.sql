...
select pkg_budget.get_category_monthly_total(
           p_household_id => 1, 
           p_category_id  => 5, 
           p_year         => 2023, 
           p_month        => 10
...
begin
    check_and_delete_inactive.deactivate_stale_users;
end;
...
begin
    provider_check_pkg.display_provider_accounts('Brainsphere');
end;
...
begin
    proc_top_categories(p_user_id => 1, p_year => 2023, p_month => 10);
end;
...
begin
    proc_user_cashflow(p_user_id => 1, p_year => 2023);
end;
...
select fn_get_monthly_expenses(
    p_user_id => 5, 
    p_year    => 2020, 
    p_month   => 10
) as monthly_total
...
select fn_savings_progress(p_user_id => 1, p_saving_id => 10) as progress_percent
from dual;
...
select calculate_total_income(1) as total_income 
from dual;
...
select calculate_total_income(1) as "income" from dual;
...
select get_category_info('Transport') as category_desc 
from dual;
